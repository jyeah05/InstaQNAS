...
class ResNet224_HW(ResNet):

    def __init__(self, block, layers, num_classes=1000):
        self.inplanes = 64//4
        super(ResNet224_HW, self).__init__()
    ...
    def seed(self, x):
        ...
        x = self.maxpool(self.relu2(self.bn2(self.conv2(x))))
        return x
...
class Policy32(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.logit(x).view(-1, self.num_blocks, self.num_of_actions)
        probs = F.softmax(x, dim=2)
        return probs, None
    def forward_feature(self, x):
        x = self.features.forward_full(x)
        return x
class Policy224_HW(nn.Module):

    def __init__(self, layer_config=[1, 1, 1, 1], num_blocks=17, num_of_actions=5):
        super(Policy224_HW, self).__init__()
        self.num_blocks = num_blocks
        self.num_of_actions = num_of_actions
        self.features = ResNet224_HW(base.BasicBlock, layer_config, num_classes=1000)
    ...
    def load_state_dict(self, state_dict):
        ...
        for k, v in state_dict.items():
            if k.startswith('features.fc'):
                continue
            new_state_dict[k] = v
        return super(Policy224_HW, self).load_state_dict(new_state_dict)
...
class Policy300(nn.Module):
    def __init__(self, layer_config=[1, 1, 1, 1], num_blocks=17, num_of_actions=5):
        super(Policy300, self).__init__()
        self.num_blocks = num_blocks
        self.num_of_actions = num_of_actions
        self.features = ResNet224(base.BasicBlock, layer_config, num_classes=1000)
        self.features.avgpool = nn.AvgPool2d(4)
