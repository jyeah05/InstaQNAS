...
class InstaNas(nn.Module):
    def forward(self, x, policy, drop_path_prob=0, is_ssd=True):
        ...
        for expansion, out_planes, num_blocks, stride in self.cfg:
            for idx in range(num_blocks):
                ...
                delta_flop = []
                feature_map = []
                for i, j in zip((0,1,2,3), range(action.size(1))):
    ...
    def _make_layers_QT(self, in_planes, version, full_pretrain):
        layers = []
        for expansion, out_planes, num_blocks, stride in self.cfg:
            strides = [stride] + [1]*(num_blocks-1)
            for stride in strides:
                block = [self._make_action_QT(in_planes, out_planes, expansion, stride, i, version, full_pretrain) for i in range(self.num_of_actions)]
                block = nn.ModuleList(block)
                layers.append(block)
                in_planes = out_planes
        self.num_of_layers = len(layers)
        print(" [*] Total num of layers: ", self.num_of_layers)
        return nn.Sequential(*layers)
    def _make_action_QT(self, inp, oup, _, stride, id, version, full_pretrain):
        if version == 'V2':
            if id == 0:  # InvertedResBlock 1 bit
                action = base.QuantInvertedResBlock(inp, oup, stride, kernel=3, expansion=1, wbit=1, abit=4, full_pretrain=full_pretrain)
            elif id == 1:  # InvertedResBlock 2 bit
                action = base.QuantInvertedResBlock(inp, oup, stride, kernel=3, expansion=1, wbit=2, abit=4, full_pretrain=full_pretrain)
            elif id == 2:  # InvertedResBlock 4 bit
                action = base.QuantInvertedResBlock(inp, oup, stride, kernel=3, expansion=1, wbit=4, abit=4, full_pretrain=full_pretrain)
            elif id == 3:  # InvertedResBlock 8 bit
                action = base.QuantInvertedResBlock(inp, oup, stride, kernel=3, expansion=1, wbit=8, abit=8, full_pretrain=full_pretrain)
            elif id == 4:  # FP block
                action = base.QuantInvertedResBlock(inp, oup, stride, kernel=3, expansion=1, wbit=32, abit=32, full_pretrain=True)
            else:
                raise ValueError(" [*] No such action index")
        else:
            if id == 0:  # InvertedResBlock 1 bit
                action = base.QuantMBBlock(inp, oup, stride, kernel=3, expansion=1, wbit=1, abit=4, full_pretrain=full_pretrain)
            elif id == 1:  # InvertedResBlock 2 bit
                action = base.QuantMBBlock(inp, oup, stride, kernel=3, expansion=1, wbit=2, abit=4, full_pretrain=full_pretrain)
            elif id == 2:  # InvertedResBlock 4 bit
                action = base.QuantMBBlock(inp, oup, stride, kernel=3, expansion=1, wbit=4, abit=4, full_pretrain=full_pretrain)
            elif id == 3:  # InvertedResBlock 8 bit
                action = base.QuantMBBlock(inp, oup, stride, kernel=3, expansion=1, wbit=8, abit=8, full_pretrain=full_pretrain)
            elif id == 4:  # FP block
                action = base.QuantMBBlock(inp, oup, stride, kernel=3, expansion=1, wbit=32, abit=32, full_pretrain=True)
            else:
                raise ValueError(" [*] No such action index")
        return action
...
class QT_MobileNet_V1_224(InstaNas):
    ...
    def __init__(self, config=None, num_classes=1000, full_pretrain=True):
        ...
        self.layers = self._make_layers_QT(in_planes=32, version='V1', full_pretrain=full_pretrain)
        self.conv2 = None
        self.linear = base.QLinearBlock(1024, self.num_classes, wbit=self.wbit, abit=self.abit, weight_only=False, full_pretrain=full_pretrain)
        self._profile(input_size=_input_size)
...
class QT_MobileNet_V1_300(InstaNas):
    ...
    def __init__(self, config=None, num_classes=1000, full_pretrain=True):
        ...
        self.layers = self._make_layers_QT(in_planes=32, version='V1', full_pretrain=full_pretrain)
        self.conv2 = None
        self.linear = base.QLinearBlock(1024, num_classes, wbit=self.wbit, abit=self.abit, weight_only=False, full_pretrain=full_pretrain)
